...
```
METAAPI_ACCOUNT_ID=sl-tp-sync
```

### 2. MT5 ICMarkets Demo (MT5-52565616)
```
METAAPI_ICMARKETS_ACCOUNT_ID=sl-tp-sync
```
...
sed -i 's/METAAPI_ACCOUNT_ID=.*/METAAPI_ACCOUNT_ID=sl-tp-sync/' /app/backend/.env

# ICMarkets Demo ID setzen
sed -i 's/METAAPI_ICMARKETS_ACCOUNT_ID=.*/METAAPI_ICMARKETS_ACCOUNT_ID=sl-tp-sync/' /app/backend/.env
